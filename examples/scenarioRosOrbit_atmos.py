...
from Basilisk.utilities import SimulationBaseClass, RigidBodyKinematics, macros, unitTestSupport, vizSupport, simIncludeThruster, simIncludeGravBody, orbitalMotion
...
def run(liveStream=False, broadcastStream=False, simTimeStep=0.1, simTime=60.0, accelFactor=1.0, thrRate=10.0, vizRate=30.0):
    ...
    for i in range(num_spacecraft):
        ...
        scObject_i.ModelTag = atmos_name
        ...
        hillFrameMRP = get_hill_frame_attitude(rN, vN)
        hillFrameMRP = RigidBodyKinematics.C2MRP(
            RigidBodyKinematics.PRV2C([0, 90 * macros.D2R, 0.0])
            @ RigidBodyKinematics.MRP2C(hillFrameMRP)
        )
        ...
        ros_bridge.add_ros_subscriber('CmdForceBodyMsgPayload', 'CmdForceBodyMsgOut', 'cmd_force', atmos_name)
        ros_bridge.add_ros_subscriber('CmdTorqueBodyMsgPayload', 'CmdTorqueBodyMsgOut', 'cmd_torque', atmos_name)
        ros_bridge.add_ros_publisher('SCStatesMsgPayload', 'SCStatesMsgIn', 'sc_states', atmos_name, max_rate=10)
        ros_bridge.add_ros_publisher('THRArrayCmdForceMsgPayload', 'THRArrayCmdForceMsgIn', 'thr_array_cmd_force', atmos_name, max_rate=10)
        ros_bridge.add_ros_publisher('HillRelStateMsgPayload', 'HillRelStateMsgIn', 'hill_trans_state', atmos_name, max_rate=20)
        ros_bridge.add_ros_publisher('AttGuidMsgPayload', 'AttGuidMsgIn', 'hill_rot_state', atmos_name, max_rate=20)
    ...
    for i in range(num_spacecraft):
        ...
        ros_bridge_i = getattr(ros_bridge, atmos_name)
